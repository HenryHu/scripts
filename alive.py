...
def get_window_list(window_class):
    try:
        output = subprocess.check_output('xdotool search --onlyvisible --class %s' % window_class, shell=True)
        return output.strip().split()
    except:
        return []

def window_large_enough(window_id, width, height):
    output = subprocess.check_output('xdotool getwindowgeometry --shell %s' % window_id, shell=True)
